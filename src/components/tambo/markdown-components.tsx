...
const CodeHeader = ({
  ...
  return (
    <div className="flex items-center justify-between gap-4 rounded-t-md bg-container px-4 py-2 text-sm font-semibold text-foreground">
      <span className="lowercase text-foreground">{language}</span>
      <button
...
> => ({
  ...
  a: ({ href, children }) => (
    <a
      ...
      className="text-foreground font-medium px-1.5 py-0.5 rounded-md bg-accent hover:bg-accent/80 hover:underline transition-colors inline-flex items-center gap-1.5"
    >
      {children}
